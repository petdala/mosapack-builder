...
async (page) => {
  ...
  const darkImagePath = '$DARK_IMAGE';
  const blocked = [
  ...
  async function step3Metrics(width, height) {
    ...
    return page.evaluate(() => {
      ...
      return {
        ...
        ctaButtons: ctaButtons.map((node) => ({ text: node.innerText.trim(), bg: getComputedStyle(node).backgroundColor, color: getComputedStyle(node).color, height: node.getBoundingClientRect().height })),
        filledButtons,
  ...
  assert(/rgb\\((0, 127, 114|8, 184, 148|11, 191, 146)/.test(desktop.ctaButtons[0].bg), 'primary CTA is not teal: ' + desktop.ctaButtons[0].bg);
  assert(desktop.filledButtons.filter((button) => /Request my free proof/.test(button.text)).length === 1, 'primary filled CTA missing');
