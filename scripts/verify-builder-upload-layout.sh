...
grep -q "min-height: clamp(470px, 54vh, 560px)" "$BUILDER" || fail "desktop upload min-height clamp missing"
grep -q "min-height: clamp(430px, 52vh, 500px)" "$BUILDER" || fail "mobile upload min-height clamp missing"
...
grep -q "JPG or PNG · up to 20MB. Clear faces, pets, and portraits work best." "$BUILDER" || fail "upload helper copy missing"
