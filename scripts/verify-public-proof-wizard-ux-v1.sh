...
grep -q "Create your custom mosaic proof" "$BUILDER" || fail "upload hero copy missing"
grep -q "Your buildable mosaic preview" "$BUILDER" || fail "preview step title missing"
...
grep -q "value=\"premium_display_quote\"" "$BUILDER" || fail "premium_display_quote format interest missing"
