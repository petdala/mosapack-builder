...
Low-confidence sensitive identifier hits: 378
...
## Additional Task-Specific Scripts Run Manually

- `bash scripts/verify-proof-ops-paused-payment.sh`: PASS
- `bash scripts/verify-d1-proof-credit.sh`: PASS
- `bash scripts/verify-buildable-proof-output.sh`: PASS
- `bash scripts/verify-production-constants-schema.sh`: PASS
- `bash scripts/verify-canonical-design-export.sh`: PASS
- `bash scripts/verify-generate-kit-pack.sh`: PASS

`verify-generate-kit-pack.sh` confirmed normal and Gate A PDF generation, non-empty manifests, Gate A bleed values `0.03` and `0.05`, Gate A PDF page count `5`, and PDF text containing `MosaPack`, `MP-FH24A`, `Actual Size`, `Measure me: 1.000 in`, `Vertical check: 1.000 in`, `feed/skew fiducials`, and the multi-sheet section note.

Fresh artifact validation also confirmed `/tmp/mosapack-gate-a-pdf-qa-79bfe33-fresh/first-hello-gate-a.manifest.json` includes `calibration.horizontal_bar_y_in = 0.4`, `calibration.vertical_bar = true`, `feed_fiducials.y_from_top_in = 0.22`, `gate_a_mode = true`, and bleed values `0.03` / `0.05`.
