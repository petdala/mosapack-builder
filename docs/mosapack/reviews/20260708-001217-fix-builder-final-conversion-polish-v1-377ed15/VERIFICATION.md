...
Low-confidence sensitive identifier hits: 537
...
/ -> 200
/builder/ -> 200
Suspicious strings in /builder/:
10535:      const token = (source.slice(0, 6) || 'PREVIEW').padEnd(6, '0').slice(0, 6);
/contact/ -> 200
...
MISSING live form mosapack-waitlist on /
MISSING hidden form-name for mosapack-waitlist on /
/builder/ form check -> 200 (mosapack-save-design)
/contact/ form check -> 200 (mosapack-contact)
Live exposure check found suspicious strings. Do not deploy, rollback, or continue to A4 until resolved.
```
Result: FAIL (exit 1)
