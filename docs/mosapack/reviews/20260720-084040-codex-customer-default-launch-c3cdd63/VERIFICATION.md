...
Generated: 20260720-084040

## Customer-default Launch Verification

- Browser QA: 17 passed. This includes fixed goldens, connected photo corpus guardrails, adaptive default displayed-equals-stored, fixed escape-hatch displayed-equals-stored, hybrid singles expansion, grout inside mergeable 2x2 regions, actual size/price data, and 390x844 overflow.
- Post-guard targeted QA: 2 displayed-equals-stored tests passed; fixed proof warning regression check passed; quality-pipeline module/UI tests 5 passed.
- Fulfillment/bridge QA: 31 passed, including adaptive Lab/role/seed/gamut propagation and a grid-squared singles cell map.
- Build and TypeScript: passed. Lint passed with pre-existing warnings only.
- Public artifact parity: `public/builder/index.html` and all referenced assets match the final `dist` output.
- Protected blobs: `mosaic.ts` `de050add38449ac88da40f77c3c315fdaf2dc86f`; `palette.ts` `c58483662aaf30b36e17a482e41b0e1ef3df428b`.
- Defaults: no params enables quality pipeline, intelligence, and adaptive palette. Debug opt-outs: `qualityPipeline=0`, `qualityIntelligence=0`, and `paletteMode=fixed`.
- Real child-photo visual: 48x48, Gallery-52, 19.2in board, 2,304 singles, about 3 hr 51 min, $119. Hero and proof modal use the same data URL.
- No production deploy, merge, or remote push was performed.

The generic verifier failures captured below predate this launch branch and assert superseded builder copy/contracts or require network access. They do not supersede the focused QA above.
...
Low-confidence sensitive identifier hits: 553
